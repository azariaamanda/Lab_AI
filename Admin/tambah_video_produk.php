...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($url_youtube)) {
        ...
        if (!preg_match('/^(https?:\/\/)?(www\.)?(youtube|youtu|youtube-nocookie)\.(com|be)\//', $url_youtube)) {
            ...
            try {
                $query = "CALL sp_insert_video_produk(:url)";
                $stmt = $koneksi->prepare($query);
                $stmt->execute([':url' => $url_youtube]);
                $success = "Video produk berhasil ditambahkan!";
                // Reset form
                $_POST = [];
...
<head>
    ...
    <title>Tambah Video Produk</title>
...
<body>
    <div class="container mt-5">
        <div class="row justify-content-center">
            <div class="col-md-6">
                <h1 class="mb-4">Tambah Video Produk Baru</h1>
                ...
                <form method="POST">
                    <div class="mb-3">
                        ...
                        <input type="url" class="form-control" id="url_youtube" name="url_youtube" 
                            placeholder="https://youtu.be/xxxxx" required>
